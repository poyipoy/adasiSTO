...
<div class="enterprise-toolbar">
    ...
    <button class="btn btn-success" type="button" id="exportExcel" onclick="exportDirectly('excel')">Export Excel</button>
    <button class="btn" type="button" id="exportPdf" onclick="exportDirectly('pdf')">Export PDF</button>
</div>
...
<script>
    ...
    function queueExport(format) {
        ...
        .then(payload => {
            const exportId = Number(payload.data?.id);
            if (exportId) {
                pendingAutoDownloadExportIds.add(exportId);

                setTimeout(() => {
                    if (pendingAutoDownloadExportIds.has(exportId)) {
                        exportDirectly(format);
                        Swal.fire({
                            toast: true,
                            position: 'bottom-end',
                            ...
                            timer: 3000,
                            icon: 'info',
                            title: 'Export worker belum selesai, download langsung dimulai.'
                        });
                    }
                }, 15000);
            }
    ...
    function exportDirectly(format) {
        const params = new URLSearchParams(filters()).toString();
        const url = (format === 'excel' ? directExportExcelUrl : directExportPdfUrl) + (params ? `?${params}` : '');
    ...
    function setupBackConfirmation() {
        registerLeaveConfirmation(isEditorDirty, 'Perubahan belum disimpan. Yakin ingin keluar?');
    }

    function disableBackConfirmation() {
        unregisterLeaveConfirmation();
    }
    ...
    const leaveConfirmState = {
        enabled: false,
        message: 'Perubahan belum disimpan. Yakin ingin keluar?',
        getDirty: null,
    };

    function registerLeaveConfirmation(getDirty, message = 'Perubahan belum disimpan. Yakin ingin keluar?') {
        leaveConfirmState.enabled = true;
        leaveConfirmState.getDirty = getDirty;
        leaveConfirmState.message = message;
    ...
    function unregisterLeaveConfirmation() {
        leaveConfirmState.enabled = false;
        leaveConfirmState.getDirty = null;
    }

    function shouldConfirmLeave() {
        return leaveConfirmState.enabled
            && typeof leaveConfirmState.getDirty === 'function'
            && leaveConfirmState.getDirty();
    }
    ...
    window.addEventListener('beforeunload', event => {
        ...
        event.preventDefault();
        event.returnValue = leaveConfirmState.message;
        return leaveConfirmState.message;
    });

    window.addEventListener('popstate', event => {
        if (!shouldConfirmLeave()) return;

        const confirmed = window.confirm(leaveConfirmState.message);

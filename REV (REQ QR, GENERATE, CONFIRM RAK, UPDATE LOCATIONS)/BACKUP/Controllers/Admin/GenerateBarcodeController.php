...
class GenerateBarcodeController extends Controller
{
    ...
    public function index(): View
    {
        ...
        return view('admin.generate-barcode', [
            ...
            'locations' => \App\Models\Location::active()->orderBy('name')->limit($filterLimit)->get(),
            'materials' => MasterMaterial::active()->orderBy('material_code')->limit($filterLimit)->get(),
        ]);
    }

    /**
     * Store a new BarcodeRequest created manually by admin.
     * Status will be 'pending' so it can be batch-generated.
     */
    public function store(Request $request): JsonResponse
    {
        $validated = $request->validate([
            'material_code' => ['required', 'string', 'exists:master_materials,material_code'],
            'shape_code'    => ['required', 'string', 'in:RF,RR'],
            ...
            'material_code.required' => 'Material wajib dipilih.',
            'material_code.exists'   => 'Material tidak ditemukan.',
            'shape_code.required'    => 'Jenis wajib dipilih.',
            'shape_code.in'          => 'Jenis harus RF (Flat) atau RR (Round).',
            ...
            'plant_id.exists'        => 'Plant tidak valid.',
            'location_id.required'   => 'Lokasi wajib dipilih.',
            'location_id.exists'     => 'Lokasi tidak valid.',
        ]);

        // Nullify dimension fields irrelevant to the chosen shape
        if ($validated['shape_code'] === 'RF') {
        ...
        $this->activityLog->record(
            user: Auth::user(),
            action: 'admin.create_barcode_request',
        ...
        return response()->json([
            'success' => true,
            'message' => 'Barcode request berhasil ditambahkan dengan status Pending.',
        ]);
    }

    /**
     * Get locations for a given plant (used by the Add Barcode modal).
     */
    public function locationsByPlant(Request $request): JsonResponse
    {
    ...
    public function datatable(Request $request): JsonResponse
    {
        ...
        if ($locationName = $request->input('filter_location')) {

...
class ScanService
{
    ...
    public function isDuplicate(string $barcodeMaterial, ?int $plantId = null, ?int $locationId = null): bool
    {
        return $this->duplicateCount($barcodeMaterial, $plantId, $locationId) > 0;
    }

    public function duplicateCount(string $barcodeMaterial, ?int $plantId = null, ?int $locationId = null): int
    {
    ...
    public function store(User $user, array $payload): array
    {
        ...
        try {
            $result = DB::transaction(function () use ($user, $payload, $activeSto) {
                ...
                $duplicateCount = $this->duplicateCount($parsed['barcode_material'], $payload['plant_id'] ?? null, $payload['location_id'] ?? null);
                if ($duplicateCount > 0 && empty($payload['force_save'])) {
                    return [
                        'success' => false,
                        'status' => 409,
                        'duplicate' => true,
                        'message' => "Barcode {$parsed['barcode_material']} (Material: {$parsed['material_name']}) sudah pernah discan sebanyak {$duplicateCount} kali. Tetap simpan?",
                    ];
                }
    ...
    public function historyQuery(User $user, array $filters = []): Builder
    {
        ...
        if (!empty($filters['barcode_material'])) {
            $query->where('scan_results.barcode_material', strtoupper(trim($filters['barcode_material'])));
        }

        if (!empty($filters['material_code'])) {
            $query->where('scan_results.material_code', strtoupper(trim($filters['material_code'])));
        }

        if (!empty($filters['plant_id'])) {
            $query->where('scan_results.plant_id', (int) $filters['plant_id']);
        }

        if (!empty($filters['location_id'])) {
            $query->where('scan_results.location_id', (int) $filters['location_id']);
        }
    ...
    public function historyFilterOptions(User $user): array
    {
        ...
        return \Illuminate\Support\Facades\Cache::remember("user_{$user->id}_history_filters", 300, function () use ($user, $limit) {
            return [
    ...
    public function updateByAdmin(User $admin, int $id, array $payload): ScanResult
    {
        return DB::transaction(function () use ($admin, $id, $payload) {
            ...
            $stoCode = $this->activeStoService->active();
            if (!$stoCode) {
                throw new \Exception(ActiveStoService::NO_ACTIVE_STO_MESSAGE);
            }
    ...
    public function storeByAdmin(User $admin, array $payload): array
    {
        ...
        $duplicateCount = $this->duplicateCount($barcodeMaterial, $payload['plant_id'] ?? null, $payload['location_id'] ?? null);
        if ($duplicateCount > 0 && empty($payload['force_save'])) {
            ...
            return [
                ...
                'status' => 409,
                'duplicate' => true,
                'message' => "Barcode {$barcodeMaterial} (Material: {$matName}) sudah pernah discan sebanyak {$duplicateCount} kali. Tetap simpan?",
            ];
        }
    ...
    public function storeMaterialDoubleScan(User $admin, array $payload): array
    {
        ...
        $duplicateCount = $this->duplicateCount($parsed['barcode_material'], $payload['plant_id'], $payload['location_id']);
        if ($duplicateCount > 0 && empty($payload['force_save'])) {
            return [
                'success' => false,
                'status' => 409,
    ...
    public function serializeScanForDatatable(ScanResult $scanResult): array
    {
        return [
            'id' => $scanResult->id,
            'barcode_material' => $scanResult->barcode_material,
